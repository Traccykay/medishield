...
| MariaDB concurrency integration | Opt-in destructive tests against the allowlisted `medishield_ui_account_test`: verify audit migration/concurrent append integrity and prove that two contending lab-result submissions cannot both claim one pending request. | `$env:MEDISHIELD_MARIADB_AUDIT_TEST='1'; vendor\bin\phpunit --filter 'MariaDb.*ConcurrencyTest'` |
