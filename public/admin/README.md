...
- **The audit log is append-only.** Even an admin can only *read* it here
  (`AuditLogger::page()`); filtering never bypasses full-chain verification, and
  there is no code path that edits or deletes entries.
