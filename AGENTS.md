...
- Audit logs are append-only and must use the HMAC-SHA256 hash chain through `AuditChain` / the audit helper; never edit or delete audit rows in app code.
  - The ONLY exception is the PII retention scrub: `audit_logs.attempted_identifier` (the email typed on a failed login) is held OUTSIDE the hash chain and is nulled by the privileged maintenance task `scripts/purge-audit-pii.php` (class `Audit\AuditRetention`), governed by `audit.pii_retention_days`. App/request code must NEVER perform this scrub — it nulls only that one non-chained column, never deletes rows, and must keep local chain verification at `PASS`. Any new audit field that should be scrubbable must likewise be kept out of `AuditChain` canonicalization.
- Every protected page must enforce server-side `require_login()`, `require_role(...)`, and object-level ownership / assignment / queue checks. Hidden UI is not authorization.
  - Doctor clinical access requires both a current active `patient_assignments` row and ownership of the active visit. Visit ownership alone must never preserve access after an administrator revokes the patient assignment.
...
- Use spec section 25 naming exactly:
  ...
  - Audit action vocabulary: `LOGIN_SUCCESS`, `LOGIN_FAILED`, `LOGOUT`, `OTP_SENT`, `OTP_VERIFIED`, `OTP_FAILED`, `OTP_EXPIRED`, `ACTIVATION_SENT`, `ACCOUNT_ACTIVATED`, `PATIENT_VIEW`, `PATIENT_REGISTERED`, `VISIT_REGISTERED`, `ASSIGNMENT_CHANGED`, `VITALS_RECORDED`, `DIAGNOSIS_ADDED`, `LAB_REQUESTED`, `LAB_RESULT_UPLOADED`, `PRESCRIPTION_ISSUED`, `MEDICATION_DISPENSED`, `MEDICATION_REFUSED`, `BILLING_VIEWED`, `BILLING_CHARGE_ADDED`, `PAYMENT_RECORDED`, `WORKFLOW_ROUTING_FAILED`, `USER_CREATED`, `USER_UPDATED`, `PASSWORD_RESET`, `ACCOUNT_LOCKED`, `ACCOUNT_UNLOCKED`, `CSRF_REJECTED`, `SQL_INJECTION_ATTEMPT`, `XSS_ATTEMPT`, `UNAUTHORIZED_ACCESS`, `AUDIT_LOGS_VIEWED`, `INTEGRITY_VERIFIED`, `AUDIT_PII_SCRUBBED`, `AUDIT_GRANTS_VERIFIED`, `EMERGENCY_ACCESS_GRANTED`, `EMERGENCY_ACCESS_DENIED`, `EMERGENCY_ACCESS_VIEWED`, `EMERGENCY_ACCESS_REVIEWED`, `EMERGENCY_ACCESS_REVOKED`.
...
- Apache's `DocumentRoot` must point to `public/`. The repository-root `.htaccess` is a fail-closed fallback for XAMPP subfolder deployments, not a substitute for the dedicated public web root.
