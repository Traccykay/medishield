...
| `AuthServiceTest.php` | `Auth/AuthService` — generic unknown/wrong-password behavior, password-confirmed lockout disclosure, attribution, anomaly flags, and lock audit action. |
...
| `AuditLoggerTest.php` | `Audit/AuditLogger` — v2 appends, authenticated key-check/head enforcement, correct-key tamper versus wrong-key classification, complete physical sequence coverage, verified anomaly/keyword filtering and pagination, format downgrade precedence, same-snapshot recent-row quarantine, local and partial PASS/FAIL/UNKNOWN verification, caller-transaction rejection, two file-backed SQLite connections, and non-chained `attempted_identifier`. |
